	...
	<div class="card flex-1">
		<div class="flex items-center justify-between">
			...
			<div class="hidden items-center gap-4 lg:flex">
				<span class="font-medium">Total: {formatter.format(data.items.total)}</span>
		...
		{:else}
			<div class="grid h-full place-items-center text-2xl text-neutral-400">No items found</div>
		{/if}
		<Pagination total={data.items.total} perPage={data.items.limit} defaultPage={data.items.page} />
	</div>
</WithBasketSummary>
...
<style lang="postcss">
	...
	td {
		@apply items-center px-4 py-1.5 align-top md:grid md:text-left md:first:pt-6 md:last:pb-6;
	}
