		...
		<Container style={{ display: 'flex', backgroundColor: '#f9fafb', maxWidth: '100%' }}>
			<Container style={main}>
				...
				<Container style={content.self}>
					<Container>
						<Heading as="h2" style={content.title}>You are one step away</Heading>
						<Text style={{ textAlign: 'center' }}>
							Thank you for choosing RENZ Online Food Ordering! We're thrilled to have you join our
							platform.
						</Text>
						<Text style={{ textAlign: 'center' }}>
							Verification is quick and easy. Simply click on the link below to confirm your email.
						</Text>

						<Container style={{ display: 'flex', width: '100%' }}>
							<Link href={`${BASE_URL}/verifyEmail?token=${token}`} style={content.trackOrder.link}>
								<Text style={content.trackOrder.label}>Verify Email</Text>
							</Link>
						</Container>
